...
#    docker build --squash="true" -t satrack-vehicles .
#
#    Build FAST (rebuild) image (cache; >280MB, build time ~360s, rebuild time ~80s):
#    docker build -t satrack-vehicles .
#
#    Clean (remove intermidiet images):
#    docker rmi -f $(docker images -f "dangling=true" -q)
#
#    Run image (on localhost:8080):
#    docker run --name satrack-vehicles -p 8080:80 satrack-vehicles &
#
#    Run image as virtual host (read more: https://github.com/jwilder/nginx-proxy):
#    docker run -e VIRTUAL_HOST=satrack-vehicles.your-domain.com --name satrack-vehicles satrack-vehicles &
